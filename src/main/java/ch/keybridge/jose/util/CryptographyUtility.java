...
public class CryptographyUtility {
   ...
   * @return bytes returned by the Cipher.
   * Always contains the ciphertext but may also contain appended authentication tag bytes.
  ...
  /**
   * Encrypt with additional algorithm specification and additional authenticated data
   *
   ...
   * @throws GeneralSecurityException in case of failure
   */
  public static byte[] encrypt(byte[] payload, Key key, String algo, AlgorithmParameterSpec spec, byte[]
      additionalAuthenticationData) throws GeneralSecurityException {
  ...
  /**
   * Decrypt with additional algorithm specification and additional authenticated data
   *
   * @param ciphertext                   ciphertext bytes
   * @param key                          key
   * @param algo                         JCA algorithm name for the Cipher
   ...
   * @throws GeneralSecurityException in case of failure
   */
  public static byte[] decrypt(byte[] ciphertext, Key key, String algo, AlgorithmParameterSpec spec, byte[]
      additionalAuthenticationData) throws GeneralSecurityException {
    Cipher cipher = Cipher.getInstance(algo);
  ...
  /**
   * Wrap (encrypt) key.
   * @param payloadKey a valid Key which will be wrapped (encrypted) using another key
   ...
   * @param key the wrapping/encryption key
   * @param algo JCA algorithm name for the Cipher
   * @param keyAlgo JCA algorithm name for the key. E.g. for an AES SecretKey this would be "AES"
